...
test("the auth schema ships outside the globbed directory", () => {
  ...
  // App migrations (the Wire schema, …) glob and apply normally; the auth
  // schema must stay in migrations/auth/ until sign-in is deliberately turned
  // on and the copy is placed next to them.
  const globbed = readdirSync(migrationsDir).filter(isMigrationFile);
